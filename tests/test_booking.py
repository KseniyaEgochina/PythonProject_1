...
def test_update_booking_with_token(booking_client, created_booking, updated_booking_payload, auth_headers):
    booking_id = created_booking['bookingid']
    update_response = booking_client.update_booking(
        booking_id,
        updated_booking_payload.build(),
        auth_headers
    )
    ...
    assert updated_data['bookingdates']['checkin'] == updated_booking_payload.bookingdates.checkin
...
def test_create_and_verify_bookings(booking_client, valid_booking_payload, headers, auth_headers):
    """Тест создания и проверки бронирований"""
    firstname = valid_booking_payload.firstname
    print(f"\n=== Начало теста для имени: {firstname} ===")

    initial_response = requests.get(f"{booking_client}/booking", params={"firstname": firstname})
    ...
    response = requests.get(f"{booking_client}/booking", params={"firstname": firstname})
    ...
    for booking_id in created_ids:
        ...
        assert delete_response.status_code in (200, 201, 204), f"Ошибка удаления {booking_id}"
